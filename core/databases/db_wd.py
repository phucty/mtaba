...
from core.databases import db_core
...
class DBWikidata(db_core.DBCore):
    __slots__ = "db_redirect", "db_redirect_of", "db_label", "db_labels", "db_descriptions", "db_aliases", "db_claims_ent", "db_claims", "db_sitelinks", "db_claim_ent_inv", "db_qid_trie"

    def __init__(self, db_file: str = cf.DIR_WIKIDATA_ITEMS_JSON, readonly=True):
        super().__init__(db_file=db_file, max_db=11, map_size=cf.SIZE_1GB * 100, readonly=readonly)
        self.db_redirect = self.env.open_db(b"db_redirect", integerkey=True)
        self.db_redirect_of = self.env.open_db(b"db_redirect_of", integerkey=True)
        self.db_label = self.env.open_db(b"db_label", integerkey=True)
        self.db_labels = self.env.open_db(b"db_labels", integerkey=True)
        self.db_descriptions = self.env.open_db(b"db_descriptions", integerkey=True)
        self.db_aliases = self.env.open_db(b"db_aliases", integerkey=True)
        self.db_claims_ent = self.env.open_db(b"db_claims_ent")
        self.db_claims = self.env.open_db(b"db_claims", integerkey=True)
        self.db_sitelinks = self.env.open_db(b"db_sitelinks", integerkey=True)
        self.db_claim_ent_inv = self.env.open_db(b"db_claim_ent_inv")
    ...
    def build_haswbstatements(self, buff_limit=cf.SIZE_512MB, step=10000):
        ...
        for k, v in tqdm(invert_index, desc="Save db", total=len(invert_index)):
            ...
            if tmp_k != tail_k:
                ...
                tail_kv_list = []
            tail_v.update(v)
            k, v = db_core.serialize(k, v, bytes_value=cf.ToBytesType.INT_BITMAP)
    ...
    def build_from_json_dump(self, json_dump=cf.DIR_DUMP_WD, n_process=1, step=1000):
        ...
        def save_buff(buff):
            for attr in buff.keys():
                ...
                if attr != "claims_ent":
                    buff[attr] = [
                        (db_core.serialize_key(k, integerkey=True), v) for k, v in buff[attr]
                    ]
        ...
        if n_process == 1:
            for i, iter_item in enumerate(reader_wikidata_dump(json_dump)):
                ...
                for attr, value in wd_obj.items():
                            ...
                            for prop_lid, values_ent in value[
                                ...
                                if isinstance(prop_lid, int) and not any(
                                    ...
                                    value_ent = db_core.serialize_value(
                                        values_ent, bytes_value=cf.ToBytesType.INT_NUMPY
                                    )
                    ...
                    value = db_core.serialize_value(value, compress_value=compress_value)
                    buff_size += len(value)
