...
def deserialize_value(value, bytes_value=cf.ToBytesType.OBJ, compress_value=False):
    if bytes_value == cf.ToBytesType.INT_NUMPY:
    ...
    elif bytes_value == cf.ToBytesType.BYTES:
        if compress_value:
            try:
        ...
        if compress_value:
            try:
                ...
                pass
        if bytes_value == cf.ToBytesType.PICKLE:
...
class DBCore:
    __slots__ = "db_file", "max_db", "map_size", "readonly", "env"
    ...
    def copy_lmdb(self):
        ...
        :rtype:
        """
        iw.print_status(self.env.stat())
        if self.env.stat().get("map_size"):
            iw.print_status("%.2fGB" % (self.env.stat()["map_size"] % cf.SIZE_1GB))
    ...
    def get_random_key(self, db, integerkey=False):
        with self.env.begin(db=db, write=False) as txn:
            ...
            for i, k in enumerate(cur.iternext(values=False)):
                if i == random_index:
                    key = deserialize_key(k, integerkey=integerkey)
                    break
        return key
    ...
    def copy_new_file(
        ...
        print(self.env.info())
        iw.print_status("%.2fGB" % (self.env.info()["map_size"] / cf.SIZE_1GB))
        save_drive = 0
        with closing(
            lmdb.open(
                new_dir,
            ...
            for db_name_src, copy_args in db_names.items():
                ...
                org_db = self.env.open_db(db_name_src)
                is_integerkey = False
                ...
                with self.env.begin(db=org_db) as txn:
                    cur = txn.cursor()
                    buff = []
                    len_buff = 0
                    if message:
